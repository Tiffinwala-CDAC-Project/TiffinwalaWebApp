...
export default function Subscription(props) {
  ...
  const handleMealChange = (selectedMeal) => {
    if (selectedMeals.includes(selectedMeal)) {
      setSelectedMeals(selectedMeals.filter((m) => m !== selectedMeal));
    } else {
      setSelectedMeals([...selectedMeals, selectedMeal]);
    }
  
    // Check if both lunch and dinner are selected
    if (selectedMeals.includes('lunch') && selectedMeals.includes('dinner')) {
      setSelectedMeals(['lunch/dinner']); // Select "lunch/dinner"
    }
  };
  

  const areFieldsDisabled = selectedMeals.length === 0;
